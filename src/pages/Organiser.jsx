    ...
    const fetchData = async () => {
      try {
        const [statsRes, eventsRes] = await Promise.all([
          ...
          API.get("/api/events/me")
        ]);

        // FIXED: Aligned properties directly with the backend payload structure
        setStats({
          totalEventsCount: statsRes.data?.totalEvents || 0,
          tickets: statsRes.data?.totalTicketsSold || 0, // <-- Fixed: Changed from totalTickets to totalTicketsSold
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            ...
            <p className="text-gray-500 font-medium mt-2">
              Here's how your EventX portfolio is performing.
            </p>
          </div>
          ...
          {[
            {
              ...
              icon: "📅",
              color: "from-blue-50 to-indigo-50 text-indigo-700"
            },
            {
              label: "Tickets Sold",
              val: stats.tickets,
            ...
            {
              label: "Net Revenue",
              val: `₹${Number(stats.revenue).toLocaleString("en-IN")}`,
              icon: "💰",
              color: "from-orange-50 to-amber-50 text-orange-700"
            }
          ].map((item, i) => (
            <div
              key={i}
              className={`bg-gradient-to-br ${item.color} p-8 rounded-[2.5rem] border border-white shadow-sm`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white shadow-xl shadow-indigo-200">
            ...
            <p className="text-indigo-100 text-sm leading-relaxed mb-6">
              Events with high-quality images see 40% more ticket conversions.
            </p>
